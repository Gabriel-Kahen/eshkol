...
The downstream transformer currently targets exact future high-water reservations
of five additional frames for direct P1 release, six for C2 release, and eleven
for LOAD rollback. Those numbers remain downstream proof obligations: compiled
guard high-water tests and persistent allocator-failure tests must establish them
before the final pin. They are not runtime guarantees or accepted transformer
cleanup evidence. There is no 4,096-entry reserve or informal safety margin.

## Compiler-private emergency rethrow entry

A direct source-top-level fixed-arity function definition may end with the
compiler-private modifier
` :runtime-emergency-rethrow-param <formal>`. The operand must name exactly one
untyped tagged formal. At every physical function entry, before parameter boxing,
tail-call rewriting, prologue calls, or body evaluation, codegen spills the exact
incoming tagged value once and calls the existing
`eshkol_runtime_emergency_rethrow_if_v1` ABI. Exact canonical conditions 1 through
5 therefore transfer before source predicates or cleanup code can allocate;
ordinary and noncanonical values continue into the function body unchanged.

Annotated functions do not use self or outbound mutual tail-call optimization,
so every logical recursive invocation reaches a physical entry check. Variable,
typed, variadic, keyword-rest, nested/internal, `begin`-nested, included, external,
duplicate, ambiguous, and malformed uses are rejected. Malformed AST metadata is
a fatal codegen error. The modifier introduces no source import, public runtime
entry point, or function-name special case, and unannotated definitions retain
...
The focused AOT fixtures use `--no-stdlib`. The emergency-rethrow modifier fixture
also runs as a cache-disabled full-file JIT test and proves exact canonical 1..5
transfer across direct, first-class, forward, and recursive calls plus ordinary
fallthrough. Broader source-library regressions
