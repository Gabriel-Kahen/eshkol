...
| `lib/backend/eskb_writer.c`, `eskb_reader.c`, and `eshkol_vm.c` | ESKB v2 has no f32 constant kind. Runtime-shaped constant tags 11 and 34 and every other unknown tag reject; the writer does so before opening the destination. VM materialization no longer converts an unknown constant to INT64, and emission refuses `VAL_FLOAT32` instead of substituting NIL or INT64. | Implemented. The 59-byte NIL/INT64/F64/BOOL/STRING fixture matches the upstream v2 header; the F32 work added no format version or constant ID. |
...
The native KB v2 writer, the VM KB writer, and the ESKB v2 bytecode writer now
