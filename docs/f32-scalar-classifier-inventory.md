...
| `gcd`, `lcm`, `modulo`, `remainder`, `quotient` public numeric helpers | Native and VM direct/stored `modulo` and `quotient` now promote canonical f32 into their inexact DOUBLE result paths; VM stored DOUBLE inputs were corrected from separate integer casts to floored-fmod and truncated-division semantics. All-INT64 and bignum VM branches remain unchanged. Malformed/folded native carriers reject before numeric coercion. Native GCD/LCM and VM GCD/LCM still reject f32 before integer conversion. VM `remainder` promotes f32 to DOUBLE for nonzero divisors. | VM `remainder` still differs from native on an inexact zero divisor; f32 currently rejects zero. GCD/LCM integer-domain policy and `numerator` remain fail-closed. This numeric subset does not establish whole-f32 acceptance. |
