...
| `gcd`, `lcm`, `modulo`, `remainder`, `quotient` public numeric helpers | Native and VM direct/stored `modulo` and `quotient` promote canonical f32 into their inexact DOUBLE result paths. VM direct/stored `remainder` likewise promotes f32 and now raises on ±0 divisor, matching native DOUBLE and F32 behavior. All-INT64 and bignum VM branches remain unchanged. Malformed/folded native carriers reject before numeric coercion. Native GCD/LCM and VM GCD/LCM reject f32 before integer conversion. | GCD/LCM integer-domain policy and `numerator` remain fail-closed. Exact VM integer/bignum zero errors still use their historical fatal boundary; inexact zero errors are catchable. This numeric subset does not establish whole-f32 acceptance. |
