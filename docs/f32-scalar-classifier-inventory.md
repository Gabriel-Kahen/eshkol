...
| `lib/core/bignum.cpp`, `lib/core/rational.cpp` public tagged arithmetic | Arithmetic, comparison, gcd, numerator, and rational construction entry points reject tag 11 before any integer/double payload fallback. `denominator` validates canonical f32 and returns the same INT64 1 as its existing DOUBLE path. | `numerator` stays fail-closed pending native/VM DOUBLE result-kind reconciliation; other rejections need separate reviewed numeric routes. |
| `gcd`, `lcm`, `modulo`, `remainder`, `quotient` public numeric helpers | Native direct and stored routes keep the established inexact DOUBLE result for canonical f32 `modulo`/`remainder`/`quotient`; malformed/folded carriers reject before numeric coercion. Native GCD and LCM, and VM GCD/LCM, reject f32 before integer conversion. VM direct and stored `modulo`/`quotient` reject f32 before its legacy integer result path; VM `remainder` promotes f32 to DOUBLE for nonzero divisors. | VM DOUBLE `modulo`/`quotient` return INT64 where native DOUBLE returns DOUBLE, and VM direct DOUBLE `modulo` differs from its stored closure on fractional inputs. These semantics need reconciliation before VM f32 can enter those routes. VM `remainder` still differs from native on an inexact zero divisor; f32 currently rejects zero. `numerator` remains fail-closed. |
