...
| `gcd`, `lcm`, `modulo`, `remainder`, `quotient` public numeric helpers | Native and VM direct/stored `modulo` and `quotient` promote canonical f32 into their inexact DOUBLE result paths. VM direct/stored `remainder` likewise promotes f32 and now raises on ±0 divisor, matching native DOUBLE and F32 behavior. Their all-INT64 and bignum VM branches remain unchanged. Malformed/folded native carriers reject before numeric coercion. Native GCD/LCM and VM GCD/LCM reject f32 before integer conversion. VM GCD/LCM raise a catchable error on non-number, non-integral/nonfinite/out-of-range DOUBLE, INT64_MIN magnitude in the int64-only path, or overflowing LCM result before unsafe coercion. VM binary direct/stored GCD now uses its existing bignum kernel for all-exact INT64/bignum operands and normalizes a fitting answer to INT64; VM LCM still rejects wide exact inputs. Native LLVM raw, tagged/stored, and dual-primal GCD/LCM now guard the same unsafe conversions; native GCD retains exact bignum support, while native LCM explicitly rejects unsupported bignum. Valid in-range integral DOUBLE has the established exact INT64 outcome in native direct/stored calls. | GCD/LCM F32 policy remains fail-closed. R7RS inexact result kind, wide LCM, arity, and AD policy remain unresolved; `numerator` is covered above. Exact VM integer/bignum zero errors for other helpers keep their historical fatal boundary; inexact zero errors are catchable. This numeric subset does not establish whole-f32 acceptance. |
